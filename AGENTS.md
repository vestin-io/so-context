...
- Before considering work done, try to run:
 ...
 - `cargo build --locked`
 - `cargo test --locked`
- If `cargo check --locked` fails because `Cargo.lock` is stale, run `cargo check`, commit the updated `Cargo.lock`, then re-run `cargo check --locked`.
...
## 14. Dependency and Lockfile Rules
...
- New dependencies must have a clear purpose tied to one of:
 ...
 - graph/indexing
 - storage
- Any change to `Cargo.toml` that affects dependency resolution—including `[dependencies]`, workspace members, or the root `version` field—must include an updated `Cargo.lock` in the same commit.
- Version bumps: after changing `version` in `Cargo.toml`, run `cargo check`, commit both `Cargo.toml` and `Cargo.lock`. The root package version is recorded in `Cargo.lock`; CI runs `cargo check --locked` and will fail if they diverge.
- Do not gitignore `Cargo.lock` for this binary crate.
